import GoogleAdRow from "./GoogleAdRow";

type GoogleAdSetRowProps = {
	adset: any;
	adSetId: string;
	campaign: any;
	campaignId: string;
...
const GoogleAdSetRow = ({
	...
	const displayedAds = (adset.ads || []).filter((ad: any, adIdx: number) => {
		...
		tableData.timeFrames.forEach((tfInner: string) => {
			...
			// console.log("------", metricInner);
			if (!metricInner) return;
			const campaignInner = metricInner.google[0].find(
					...
					return tableData.events.map((event: any) =>
						event.metrics.map((metric: string) => {
							...
							const getSum = (field: string) =>
								displayedAds.reduce((sum: number, _ad: any, adIdx: number) => {
