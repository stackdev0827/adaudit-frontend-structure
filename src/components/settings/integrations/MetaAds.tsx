...
const MetaAds: React.FC = () => {
	...
	const fetchAds = async (
		...
		try {
			const response = await integrationApi.getMetaAdsAccounts(
				page,
				page_size,
			...
			const normalized: Ad[] = (payload.adaccounts || []).map(
				(a: any) => {
					...
					return {
						...
						adsAccountId: a.ads_account_id?.String ?? "-",
						actId: a.act_id?.String ?? "-",
						name: a.name?.String ?? "-",
						amount_spent:
							a.amount_spent?.Int64 ?? a.amount_spent ?? 0,
						...
						status: mapMetaStatus(a.status?.Int32 ?? a.status),
	...
	const handleAddAccount = async () => {
		...
		try {
			const response = await integrationApi.getMetaAdsAuthUrl("meta-ads");
	...
	// 	setIsLoading((prev) => ({ ...prev, [formId]: true }));
	// 	try {
	// 		if (isSelected) {
	// 			const response = await integrationApi.deleteTypeformWebhook(formId);
	// 			if (response.data.form_id === formId) {
	// 				setSelectedForms((prev) => prev.filter((id) => id !== formId));
	// 			}
	// 		} else {
	// 			const response = await integrationApi.createTypeformWebhook(formId);
	// 			if (response.data.form_id === formId) {
	...
	// 	} finally {
	// 		setIsLoading((prev) => ({ ...prev, [formId]: false }));
	// 	}
	// };
	...
	const syncAccounts = async () => {
		try {
			setSyncLoading(true);
			const result = await integrationApi.syncMetaAdAccount(
				selectedAccount || ""
			);
			console.log(result);
		} catch (error) {
	...
	const disconnectAccount = async () => {
		try {
			...
			await integrationApi.disconnectMetaAds(selectedAccount);
		} catch (error) {
			console.log(error);
		} finally {
	...
	const handleToggleCron = async (ad: Ad) => {
		...
		try {
			...
			if (
				typeof (integrationApi as any).updateMetaAdsCronStatus ===
				"function"
			) {
				await (integrationApi as any).updateMetaAdsCronStatus(
					ad.id,
					newStatus
				);
			} else if (
	...
	return (
		<div className="space-y-6">
			<div className="  items-center">
				...
				<Title>About Meta Ads Account</Title>
			</div>
			<Card>
