...
const AddDialog: React.FC<AddDialogProps> = ({
	...
	useEffect(() => {
		...
		setWordsToPageInput("");
		setWordsToPrevious(initialValues?.wordstoprevious || []);
		setWordsToPreviousInput("");
	}, [initialValues, open]);

	const handleOk = async () => {
		setIsLoading(true);
		// console.log(matchTypePage, matchTypePrevious, matchType);
		const AddRulePayload = {
			...
			match_type_for_previous_url:
				mode === "both"
					? matchTypePrevious === "contains"
						? 1
						: 2
					: 0,
		};
		// console.log(AddRulePayload);
		try {
			await urlRuleApi.add(AddRulePayload);
			...
			// setNotification({ message: "Failed to add rule.", type: "error" });
			console.log("error");
		} finally {
			setIsLoading(false);
		}
				...
				<DialogPanel>
					<div className="space-y-6">
						...
						<div>
							...
							>
								<SelectItem value="Lead Form Submission">
						...
						{mode === "page" && (
							<div className="bg-gray-50 rounded-lg p-4 border border-gray-200">
								...
								<TextInput
									...
									onChange={(e) =>
										setWordsToPageInput(e.target.value)
									}
									onKeyDown={(e) => {
										if (e.key === "Enter") {
											e.preventDefault();
